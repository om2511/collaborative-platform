...
export const activityService = {
  ...
  getRecentActivity: (limit = 10) => api.get(`/analytics/activity?limit=${limit}`),
  
  // Get activity for specific project
  getProjectActivity: (projectId, limit = 10) => api.get(`/analytics/projects/${projectId}/activity?limit=${limit}`)
};

export const notificationService = {
  // Get user notifications (using dashboard data as source)
  getNotifications: (limit = 5) => api.get(`/analytics/dashboard`),
  
  // Mark notification as read (placeholder - could be implemented later)
  markAsRead: (notificationId) => Promise.resolve({ success: true })
};
